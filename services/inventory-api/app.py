...
def bootstrap():
    for _ in range(40):
        try:
            ...
            time.sleep(1)
    raise RuntimeError("inventory-api database bootstrap exhausted retries")
bootstrap()
...
def health():
    try:
        with conn() as c: c.execute('SELECT 1 FROM inventory LIMIT 1')
...
def inventory(store_id:str):
    ...
    return [{'sku':r[0],'item':r[1],'unit':r[2],'stock':float(r[3]),'forecast4h':float(r[4]),'risk':'HIGH' if r[3]<r[4]*Decimal("0.75") else ('MEDIUM' if r[3]<r[4] else 'LOW')} for r in rows]
