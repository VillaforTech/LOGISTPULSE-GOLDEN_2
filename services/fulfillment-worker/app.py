...
def process_message(msg):
    ...
    if not oid:
        raise ValueError("command has no orderId")
    with conn() as c:
        row = c.execute(
    ...
    if row is None:
        raise RuntimeError("order not available yet")
    ...
    while True:
        try:
            consumer = KafkaConsumer(
                "logistpulse.orders", bootstrap_servers=KAFKA,
                group_id="kitchen-worker", auto_offset_reset="earliest",
                enable_auto_commit=False,
                value_deserializer=lambda b: json.loads(b.decode()),
            )
            break
        except Exception as exc:
            print("kafka waiting", exc)
            time.sleep(2)
    consume_commands(consumer, process_message)
